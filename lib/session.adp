...
<td><if @assessment_data.survey_p;literal@ ne t> <if @showpoints@ eq 1> <strong>#assessment.Percent_Score#:</strong>  <span>@percent_score@</span></if></if> </td></tr>
...
<multiple name="sections">
  ...
  <fieldset style="padding:10px;margin-bottom:10px"><!-- Section FieldSet -->
    <legend>@sections.title@   <if @assessment_data.survey_p;literal@ ne t> <if @showpoints@ eq 1 and @sections.max_points@ gt 0>(@sections.points@ / @sections.max_points@ #assessment.points#)</p></if> </if></legend>
...
<hr>
<if @assessment_data.survey_p;literal@ ne t and @assessment_data.show_feedback@ ne none and @assessment_score@ not nil and @assessment_score@ gt 0><strong>#assessment.Total_score#:</strong> @session_score@ / @assessment_score@ = @percent_score@%</if>
