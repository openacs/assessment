...
<tr><td><strong>#assessment.User_ID#:</strong> <span><if @show_username_p@><a href="@session_user_url@">@first_names@ @last_name@</a><if @assessment_data.anonymous_p@ eq "t"><br>#assessment.lt_bNoteb_This_assessmen#</if></if><else>#assessment.anonymous_name#</else></span></td>
<td><strong>#assessment.Attempt#:</strong> <multiple name="session_attempts"><if @session_attempts.session_id@ eq @session_id@><strong>@session_attempts.rownum@</strong></if><else><a href="session?session_id=@session_attempts.session_id@">@session_attempts.rownum@</a></else>&nbsp;&nbsp; </multiple> <if @admin_p@> (<a href="@delete_url@">#assessment.Delete_Attempts#</a>)</if></td>
