...
foreach one_choice $choices {
    util_unlist $one_choice choice_id title correct_answer_p feedback_text selected_p percent_score sort_order
    incr count
    set options ""
    if {$count < $total} {
	append options { <a href=\"item-mc-edit-choices-swap?assessment_id=$assessment_id&section_id=$section_id&as_item_id=$as_item_id&mc_id=$as_item_type_id&sort_order=$sort_order&direction=down\"><img src=\"../graphics/down\" border=\"0\" alt=\"[_ assessment.Move_Down]\"></a>}
    }
    if {$count > 1} {
	append options { <a href=\"item-edit-mc-choices-swap?assessment_id=$assessment_id&section_id=$section_id&as_item_id=$as_item_id&mc_id=$as_item_type_id&sort_order=$sort_order&direction=up\"><img src=\"../graphics/up.gif\" border=\"0\" alt=\"[_ assessment.Move_Up]\"></a>}
    }
    append options { <a href=\"item-edit-mc-choices-delete?assessment_id=$assessment_id&section_id=$section_id&as_item_id=$as_item_id&choice_id=$choice_id\"><img src=\"../graphics/delete.gif\" border=\"0\" alt=\"[_ assessment.remove_choice]\"></a>}

    if {$correct_answer_p == "t"} {
	append ad_form_code "\{infotxt.$choice_id:text(inform) \{label \"[_ assessment.Choice] $count\"\} \{value \"<img src=../graphics/correct.gif> $title$options\"\}\}\n"
    } else {
	append ad_form_code "\{infotxt.$choice_id:text(inform) \{label \"[_ assessment.Choice] $count\"\} \{value \"<img src=../graphics/wrong.gif> $title$options\"\}\}\n"
    }
