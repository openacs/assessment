...
switch -- $item_type {
    "textarea" {
        element create item-add-2  as_item_type_oq__as_item_default -label "Default value" \
            -datatype text -html {size 15} 
        element create item-add-2 as_item_type_oq__feedback_text -label "Feedback" \
            -datatype text -html {rows 5 cols 50} -widget textarea -optional
	
	form set_properties item-add-2 -action "index"
    }
    "checkbox" -
    "radiobutton" {
        element create item-add-2 as_item_type_mc__increasing_p -label "Increasing" \
            -datatype text -widget select -options {{{All or Nothing} f } {Increasing t} } -optional 

        element create item-add-2 as_item_type_mc__allow_negative_p -label "Allow negative?" \
            -datatype text -widget radio -options {{Yes t} {No f}} -value f  -optional 
        element create item-add-2  as_item_type_mc__num_correct_answers -label "Number of Correct Answers" \
            -datatype integer -html {size 7} -optional
        element create item-add-2 as_item_type_mc__num_answers -label "Number of Answers" \
            -datatype integer -html {size 7}  
        ...
        if { [template::form is_valid item-add-2] } {
            ...
            db_transaction {   
                ...
                set as_item_type_id [as::item_type_mc::new -increasing_p $as_item_type_mc__increasing_p \
                                         -allow_negative_p $as_item_type_mc__allow_negative_p \
                                         -num_correct_answers $as_item_type_mc__num_correct_answers \
                                         -num_answers $as_item_type_mc__num_answers]
                #set the relation between as_items table and as_item_type_mc table
                content::item::relate \
                    -item_id [db_string cr_item_from_revision "select item_id from cr_revisions where revision_id=:as_item_id1"] \
                    -object_id [db_string cr_item_from_revision "select item_id from cr_revisions where revision_id=:as_item_type_id"] \
                    -relation_tag {as_item_type_rel} \
                    -relation_type {cr_item_rel}
            }
    
            #redirect back to choice-add
            ad_returnredirect [export_vars -base choice-add {as_item_type_id as_item_type_mc__num_answers item_type}]
            ad_script_abort
        }
    ...
    "open_question" {
        element create item-add-2 as_item_type_oq__as_item_default -label "Item default" \
            -datatype text -html {size 15} -optional
        element create item-add-2 as_item_type_oq__feedback_text -label "Feedback" \
            -datatype text -widget textarea -html {rows 5 cols 50} -optional 
	 
        form set_properties item-add-2 -action "index"
    }
    
    "shortanswer" {
        element create item-add-2 as_item_type_sa__increasing_p -label "Increasing" \
            -datatype text -widget select -options {{{All or Nothing} f } {Increasing t} } -optional
        element create item-add-2 as_item_type_sa__allow_negative_p -label "Allow negative?" \
            -datatype text -widget radio -options {{Yes t} {No f}} -value f  -optional 
        element create item-add-2 as_item_type_mc__num_answers -label "Number of Answers" \
            -datatype integer -html {size 7} 
    }
    ...
    "imagemap" {
        element create item-add-2 as_item_type_im__increasing_p -label "Increasing" \
            -datatype text -widget select -options {{{All or Nothing} f } {Increasing t} } -optional
        element create item-add-2 as_item_type_im__allow_negative_p -label "Allow negative?" \
            -datatype text -widget radio -options {{Yes t} {No f}} -value f  -optional 
	element create item-add-2 as_item_type_mc__num_answers -label "Number of Answers" \
            -datatype integer -html {size 7} 
    }   
