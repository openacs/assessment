...
<body>
<h2>Overview</h2><p>At its core, the Assessment package defines a hierarchical
container model of a "survey", "questionnaire" or "form". This
approach not only follows the precedent of existing work; it also
makes excellent sense and no one has come up with a better
idea.</p><ul>
...
</ul><p>We choose the terms Assessment-Sections-Items-Choices over
...
broader applicability intended for this package.</p><p>As is the custom in the OpenACS framework, all RDBMS tables in
...
keeping with current OpenACS best practice.</p><p>Several of the Metadata entities have direct counterparts in the
Data-related partition of the data model. Some standards (notably
CDISC) rigorously name all metadata entities with a "_def" suffix
and all data entities with a "_data" suffix -- thus "as_item_def"
and "as_item_data" tables in our case. We think this is overkill
since there are far more metadata entities than data entities and
in only a few cases do distinctions between the two become
important. In those cases, we will add the "_data" suffix to data
entities to make this difference clear.</p><p>A final general point (that we revisit for specific entities
...
by the fact that the CR has become "the Right Way" to build OpenACS
systems. Note that one implication of this is that we can't use a
couple column names in our derived tables because of naming clashes
with columns in cr_items and cr_revisions: title and description.
Furthermore we can handle <a href="versioning">versioning</a>
and internationalization through the CR.</p><h2>Synopsis of The Data Model</h2><p>Here's a detailed summary view of the entities in the Assessment
package. Note that in addition to the partitioning of the entities
between Metadata Elements and Collected Data Elements, we identify
the various subsystems in the package that perform basic
functions.</p>
We discuss the following stuff in detail through the subsequent
pages, and we use a sort of "bird's eye view" of this global
graphic to keep the schema for each subsystem in perspective while
homing in on the relevent detail. Here's a brief introduction to
each of these section<br><ul>
<li>
<a href="as_items.html">core - items</a> entities (purple)
...
<a href="sequencing">scoring ("grading")</a> entities
(yellow-green) define how raw user responses are to be processed
into calculated numeric values for a given Assessment</li><li>
<a href="display_types.html">display</a> entities (light blue)
...
<a href="data_collection.html">session data collection</a>
