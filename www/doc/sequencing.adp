...
survey" package via hinge points defined by responses to single
items. But what if branching/skipping needs to depend on
...
determining the user's next action. This separation is made (along
with quite a few other distinctions/complexities) in the IMS
"simple sequencing" model diagrammed below). But to jump the gun a
bit, we think that actually it makes sense to combine these two
processes into a common "post-submission user input processing"
...
replaces in each of them ":value" with the actual response.<br>
...
section if the response to item(1234) "Color of your eye" was
"blue" and the response to item(4231) "Color of your hair" was
"red". Sadly we can't use such an easy way of checking the ":value"
as we do with item_checks, as we do not know which item this refers
to. Instead we store the item_id like this ":item_1234". This way
the check_sql would look like ":item_1234 == 'blue' AND :item_4231
== 'red'". Additionally other variables might be defined by the API
at a later stage,  e.g. ":percent_score", which would be
replaced by the current percentage value (aka score) that subject
had in the test so far (taken from the as_session_table). It might
be interesting to pass these variables along in the API, this
remains to be seen when actually implementing the system.<br><br>
...
post_check.<br>
</li><li>item_id - Item to call if we are in "per item" mode (all items
will be displayed on a seperate page) and it is a post_check.</li><li>Potential extension: item_list - list of item_ids that are used
