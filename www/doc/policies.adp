...
formats: standard web page, handheld gizmo, kiosk "one question at
a time", AVR over phone, etc etc</li><li>consent_required_p - whether subjects must give formal consent
...
"attestation box" and provide password to "sign"</li><li>digital_signature_p - whether in addition to the electronic
signature, the response must be hashed and encrypted</li><li>shareable_p - whether Policy is shareable; defaults to 't'
since this is the whole intent of this "repository" approach, but
authors' should have option to prevent reuse</li><li>feedback_text - where optionally some preset feedback can be
specified by the author</li><li>double_entry_p - do two staff need to enter data before it's
accepted?</li><li>require_annotations_with_rev_p - is an annotation required if a
user modifies a submitted response?</li>
</ul>
</li><li>Assessment Events (as_assessment_events) define an planned,
scheduled or intended "data collection event". It abstracts out
from Assessment Policies details that define specific instances of
an Assessment's deployment. Attributes include:
<ul>
<li>event_id</li><li>name</li><li>description</li><li>instructions</li><li>target_days_post_enroll - an interval after the "enrollment"
date which could be the time a subject is enrolled in a trial or
the beginning of a term</li><li>optimal_days_pre - along with the next attribute, defines a
