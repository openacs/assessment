...
<body>
<h2>Overview</h2><p>This topic requires special mention because it is centrally
...
modified in a controlled fashion).</p><p>During its lifetime, an Assessment may undergo revisions in the
...
data collected across all these changes.</p><p>Another type of "revision" occurs when a component (an Item
...
data need to be assembled into trans-language data sets.</p><p>Note that two orthogonal constructs are in play here:</p><ul>
...
identical text.</li>
</ul><h2>Approach</h2><p>The Content Repository (CR) in OpenACS is designed to handle
...
So the ideas here are still somewhat exploratory.</p><p>For each of the package components that need to be versioned
...
</ul><p>This pattern of dual tables is used for all components that need
...
about the Item, a new as_items_revs row is inserted.</p><p>Now here is where things become tricky, though.. Any time a
...
automagically.</p><p>So what the package must do is version both the individual
...
to pull collected data into a report).</p><p>This doesn't involve merely creating triggers to insert new
...
than the component we've just revised. Thus:</p><ul>
...
</ul><p>Another key issue, discussed in <a href="http://openacs.org/forums/message-view?message_id=167168">this
...
now a new one?</p><p>One possible way this could be defined would derive from the
...
new as_item_rev for the existing as_item.</p><p>A final point concerns the mapping tables. The OpenACS framework
...
</p><h2>Specific Versionable Entities</h2><p>Within each subsystem of the Assessment package, the following
...
each of the subsystems' pages.</p><ul>
<li>Core - Items:
<ul>
