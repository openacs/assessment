...
#    if { ! [info exists as_session_id] } {    
    ...
    set session_id [content::item::new -parent_id $folder_id -content_type {as_sessions} -name "$subject_id-$assessment_id-[exec uuidgen]" -title "$subject_id-$assessment_id-[exec uuidgen]" ]
    set as_session_id [content::revision::new \
    			-item_id $session_id \
			-content_type {as_sessions} \
			-title "$subject_id-$assessment_id-[exec uuidgen]" \
