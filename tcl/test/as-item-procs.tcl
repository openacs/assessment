    ...
    aa_run_with_teardown \
        ...
        -test_code {
            ...
            if {[string is integer $as_item_rev_id]} {
                aa_true "Item created" [db_0or1row q {
                    select title,mime_type
                      from cr_revisions
                     where revision_id=:as_item_rev_id
                }]
            } else {
                aa_log "Item creation failed with $as_item_rev_id"
            }
            set content [db_string q {
                select content
                  from cr_revisions
                 where revision_id=:as_item_rev_id
            }]
            ...
            aa_true "Item created" [db_0or1row q {
                select title,mime_type
                  from cr_revisions
                 where revision_id=:new_item_rev_id
            }]
            set content [db_string q {
                select content
                  from cr_revisions
            ...
            aa_true "Item created" [db_0or1row q {
                select title,mime_type
                  from cr_revisions
                 where revision_id=:new_rev_rev_id
            }]
            set content [db_string q {
                select content
                  from cr_revisions
